...
// Local debug helper — same pattern as delivery_controller.js
const _log = (...args) => {
...
module.exports = (sequelize) => {
    class DeliverySurgeRule extends Model {
         ...
         *
         * @param {number|null} pricingZoneId
         * @returns {{ rule: DeliverySurgeRule|null, multiplier: number }}
         */
        static async getActiveSurge(pricingZoneId = null) {
            try {
                ...
                _log(`🕐 [SURGE] Checking surge — zone=${pricingZoneId}, day=${currentDay}, time=${currentTime}`);
                ...
                if (!allActiveRules.length) {
                    _log('🕐 [SURGE] No active rules found → no surge');
                ...
                if (!matchingRules.length) {
                    _log('🕐 [SURGE] No rules match current time window → no surge');
                    return { rule: null, multiplier: 1.00 };
                }

                const winningRule  = matchingRules[0]; // already sorted by priority DESC
                const multiplier   = parseFloat(winningRule.multiplier);

                _log(`⚡ [SURGE] Active: "${winningRule.name}" → ${multiplier}x (priority ${winningRule.priority})`);

                return { rule: winningRule, multiplier };
